 ...
 *
 * V3/V4 ISOLATION: This file does NOT import from @elconv/target-v3.
 * It defines its own minimal input interface (BridgeV3Element) to accept
 * V3-shaped data without creating a package dependency.
 */

import type { V4TreeNode, V4StyleClass } from './types.js';

/**
 * Minimal V3 element shape for bridge input.
 * Defined locally to avoid importing @elconv/target-v3 (isolation rule).
 */
export interface BridgeV3Element {
  id: string;
  elType: string;
  widgetType?: string;
  settings?: Record<string, unknown>;
  elements?: BridgeV3Element[];
  isInner?: boolean;
}
