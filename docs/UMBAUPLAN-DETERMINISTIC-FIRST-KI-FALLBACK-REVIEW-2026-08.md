...
> **Nachtrag (2026-08-05), nach unabhängigem Cross-Review:** Korrektur der `visionEnhance`-Dateizahl (§4/§2.2), zweiter ungegateter Vision-Aufrufpfad ergänzt (§3-Tabelle, §4.1, §5 KI-12), Einschränkung zur Charta-Schwelle 0.85 vs. 0.60 (§2), Einschränkung zum `FixStrategyRanker`-Vorbild (§4/§5.2), zwei offene Designfragen zu `conflict`-Erkennung und Cache-Key ergänzt (§4/§3.1, §4.1), `runVisionQA`-Unsicherheit aufgelöst (§6).
...
**Einschränkung dieser Empfehlung:** 0.85 und 0.60 sind in der Charta nicht austauschbar. ≥0.85 heißt „automatisch planen"; 0.60–0.84 heißt „planen, aber Review-Hinweis pflichtig" — also ausdrücklich *kein* AI-Fallback, sondern ein menschlicher Hinweis; <0.60 heißt „kein spekulatives Pattern". Wird `minConfidence` (die Schwelle, unterhalb derer AI-Fallback ausgelöst wird) auf 0.85 gesetzt, löst künftig jede Section im gesamten 0.60–0.84-Band einen KI-Aufruf aus — ein Band, das die Charta ursprünglich für „nur Review-Hinweis, kein Fallback" vorgesehen hat. Das kann deutlich mehr Aufrufvolumen erzeugen als beabsichtigt. Alternative: `minConfidence = 0.60` (AI-Fallback nur unterhalb des gesamten Charta-Bands, im 0.60–0.84-Bereich bleibt es beim Review-Hinweis). Welche der beiden Lesarten gemeint ist, sollte bewusst entschieden werden — nicht implizit über die Wahl der Zahl 0.85.
...
| Zweiter Vision-Aufrufpfad (Fund aus Cross-Review, nicht im Plan) | `classifyByVision()` — `detect-by-vision.ts`, aufgerufen aus `component-detector.ts::detectComponentMultiLayer()` | nimmt rohe `VisionCallFn`-Callback entgegen, läuft **nicht** über `AIRouter`/`CostTracker`; einziger bekannter Aufrufer (`section-picker.ts::enhanceWithStructure()`) übergibt keine Vision-Inputs — Reichweite ungeklärt | vor KI-03 klären (siehe KI-12); sonst schützt der geplante Circuit-Breaker um `AIRouter.execute()` diesen Pfad nicht |

**Lesart dieser Tabelle:** In elf von fünfzehn Zeilen existiert bereits ein funktionierender, getesteter Baustein (die neue, letzte Zeile ist die Ausnahme — dort steht keine wiederverwendbare Lösung, sondern eine zusätzliche, ungegatete Lücke, siehe KI-12). Die eigentliche Arbeit des Plans ist zu ~70 % *Verdrahtung und Konsolidierung*, nicht Neubau. Das verschiebt auch die Risikoeinschätzung: Das größte Risiko ist nicht „baut es nicht", sondern „baut eine zweite, leicht andere Version von etwas, das schon da ist" — genau das Muster, das laut `CRITICAL-FAILURE-POINTS.md` (P6, Barrel-Kollisionen) in diesem Repo bereits mehrfach echte Bugs verursacht hat.
...
**§2.2 Rückwärtskompatibilität (`visionEnhance` → `fallback`)** — Korrekt priorisiert. **Korrektur (per Cross-Review, Grep case-insensitive verifiziert):** `visionEnhance` kommt tatsächlich nur in **fünf** Dateien vor — `wizard.ts`, `state-manager.ts`, `pipeline-runner.ts`, `analysis/pipeline.ts`, `clone-v3.ts`. In den ursprünglich mitgelisteten `cmd-wizard.ts`, `analysis/legacy-repair.ts`, `repair-block.task.ts`, `wizard-contract.contract.ts` und `schemas/wizard-contract.schema.json` kommt der String **nicht** vor (null Treffer, verifiziert). Die Migration ist damit kleiner als hier ursprünglich dargestellt — bleibt aber pro tatsächlicher Aufrufstelle einen eigenen, expliziten Testfall wert, nicht nur einen generischen Migrationstest.
...
3. **Kein Erkennungsmechanismus für `conflict` selbst definiert.** Der neue Status setzt voraus, dass irgendetwas entscheidet, wann ein AI-Ergebnis „stark genug" einem deterministischen Kandidaten widerspricht, um `conflict` statt `ok`/`uncertain` zu setzen. Für generisches `T` ist das nicht trivial (Textvergleich? Kategorien-Ungleichheit? Schwellwert auf einer Distanzfunktion?) und in keinem der beiden Reviews spezifiziert — sollte vor KI-01 geklärt werden, sonst bleibt `conflict` ein Statuswert ohne Regel, wann er gesetzt wird.
...
**Ergänzung (zweiter, unadressierter Aufrufpfad):** Ein unabhängiges Cross-Review desselben Plans fand einen zweiten Vision-Aufrufpfad, der `AIRouter` komplett umgeht: `classifyByVision()` (`detect-by-vision.ts`), aufgerufen aus `component-detector.ts::detectComponentMultiLayer()`, nimmt eine rohe `VisionCallFn`-Callback entgegen — kein Routing, kein Cost-Tracking, kein Schutz durch den in KI-03 vorgeschlagenen Circuit-Breaker. Der einzige bekannte Aufrufer (`section-picker.ts::enhanceWithStructure()`) übergibt aktuell keine Vision-Inputs, die Reichweite ist damit ungeklärt (siehe §3-Tabelle, neue Zeile, und KI-12 in §5). Vor KI-03 sollte geklärt werden, ob dieser Pfad aktiv erreichbar ist — sonst bleibt eine ungegatete Aufrufstelle bestehen, obwohl `AIRouter.execute()` abgesichert wurde.

**Offene Frage zum Cache-Check (Schritt 0):** Der oben vorgeschlagene Cache-Check vor dem Router-Aufruf braucht einen konkreten Cache-Key. Naheliegend wäre `sectionSelector + taskName + Hash(zugeschnittenes Screenshot)` — bei reiner Section-Selector/Task-Kombination ohne Bild-Hash würde eine zwischen zwei Runs veränderte Seite (z. B. nach einem Rebuild) fälschlich einen alten Cache-Treffer liefern. Das sollte vor KI-06 explizit festgelegt werden, nicht implizit im Code entstehen.
...
**Einschränkung zu diesem Vorbild:** `FixStrategyRanker` bewertet Reparaturstrategien anhand historischer Erfolgsraten — für Section-Kandidaten („ist das eine Hero-Section?") gibt es keine vergleichbare Erfolgshistorie. Übertragbar ist nur die **Technik** (gewichtete Summe, hart auf `[0,1]` geclampt), nicht die konkrete Formel oder ihre Eingabegrößen. Die im Originalplan (§5.2.2) skizzierten domänenspezifischen Signale (Position, CTA, wiederholte Kartenstruktur) bleiben der richtige fachliche Ausgangspunkt — nur eben durch denselben Clamp-Mechanismus geführt, statt eine neue Ad-hoc-Gewichtung zu erfinden.
...
| KI-12 | P0 | `target-v3/classifier` | Klären, ob `classifyByVision()`/`detectComponentMultiLayer()` (zweiter, `AIRouter`-umgehender Vision-Pfad, siehe §3-Tabelle/§4.1) aktiv erreichbar ist; falls ja, an `AIRouter`/Circuit-Breaker anschließen, falls nein, als toten Code entfernen | Repo-weite Aufrufer-Suche + ggf. Testlauf dokumentiert; Entscheidung (verdrahten/entfernen) umgesetzt |
...
- ~~**`packages/qa/src/vision-qa.ts` und `core/ai/tasks/vision-qa.task.ts`** — ob die von `CRITICAL-FAILURE-POINTS.md` (P2) behauptete Lücke „`runVisionQA` existiert nirgends" noch aktuell ist, habe ich nicht verifiziert.~~ **Nachträglich per Cross-Review verifiziert:** P2 ist überholt. `core/ai/tasks/vision-qa.task.ts` exportiert `runVisionQA` (läuft über `AIRouter`); `qa/src/vision-qa.ts::runVisionQa()` delegiert bereits dorthin, sobald ein `router` übergeben wird — mit Fallback auf den alten Direct-Fetch-Pfad, falls nicht.
