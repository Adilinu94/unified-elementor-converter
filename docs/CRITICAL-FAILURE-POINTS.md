...
**Recommended next step:** the `v4-cmd-*.ts` family (8-way `printHelp` collision + the dry-run/pipeline pairs) looks like an entire parallel CLI implementation that was added alongside the original `cli/src/*.ts` files rather than replacing them — worth checking whether `v4-cmd-*.ts` is wired into `cli/src/index.ts`'s command router at all before deciding whether to merge, delete, or keep both under disambiguated names.
