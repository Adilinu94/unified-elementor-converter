...
- **WP-seitiger Gegenpart bereits vorhanden** (verifiziert, s. Teil 5 Punkt 1): `novamira-adrianv2` registriert seit v1.1.0 `novamira-adrianv2/wpcode-{list,get,create,update,set-status,duplicate,delete,check-setup}` — `core/src/wpcode.ts` ruft diese Abilities per MCP auf, keine neue WP-seitige Snippet-Verwaltung bauen
- DoD: `target-v3` und `target-v4` importieren beide aus `core/src/wpcode.ts`, aber keines aus dem jeweils anderen Target; erzeugte Snippets werden erfolgreich per `wpcode-create`/`wpcode-update` deployt (Integrationstest gegen echte MCP-Instanz)

**Phase 55 — Framer-Preflight-Gates**
- Quelle: `scripts/preflight/{ensure-elementor-experiments,check-unframer-connectivity,verify-xml-project-match}.ts`, `preflight-check.ts`, `pro-fallback.ts`, `elementor-version.ts`, **`session-init.ts`** (278 Zeilen — Umgebungs-Readiness-Probe: MCP-Erreichbarkeit, Elementor-Version/Pro-Status, V4-Experiment-Flags `e_atomic_elements`/`e_opt_in_v4`/`e_variables`/`e_classes`, s. Teil 5 Punkt 4)
...
- DoD: 5 neue Gate-Typen im bestehenden Preflight-Framework registriert, kein neues Preflight-System daneben; `session-init.ts`s `McpCallEntry[]`-Report-Struktur als Rückgabeformat übernehmen (bewährtes Format für schrittweise MCP-Ausführung)
...
## Teil 5 — Entscheidungen (recherchiert und aufgelöst)

Alle 5 Punkte aus der Vorversion sind jetzt gegen den tatsächlichen Code von `novamira-adrianv2` (frisch geklont) verifiziert, keine offenen Fragen mehr:

1. **PHP-Ability-Injector (11 Dateien) — bereits vollständig abgedeckt, keine Portierung nötig.** Aufgeteilt in zwei Gruppen, beide bereits in `novamira-adrianv2` vorhanden:
   - *Content-Fix-Abilities* (`adrians-add-alt-text.php`, `adrians-fix-color-contrast.php`, `adrians-generate-schema-markup.php`, `adrians-generate-meta-tags.php`): alle 4 im Plugin-Code per Grep bestätigt vorhanden.
   - *Snippet-CRUD-Abilities* (`adrians-code-injector.php`, `adrians-batch-inject-snippets.php`, `adrians-{delete,get,list,update}-snippet.php`): unter den alten Dateinamen nicht mehr auffindbar, weil sie längst durch eine sauberere, klassenbasierte Implementierung ersetzt wurden: `includes/abilities/wpcode/class-wpcode-snippets.php` registriert **7 Abilities** (`list`, `get`, `create`, `update`, `set-status`, `duplicate`, `delete`) — mehr als das ursprüngliche 5-Operationen-Set —, dazu `class-wpcode-check-setup.php` (Readonly-Probe, ob WPCode aktiv ist) und `class-inject-wpcode-snippet.php`. Seit Plugin-Version 1.1.0 registriert.
   - **Konsequenz für Phase 54**: Der dort geplante `core/src/wpcode.ts`-Generator hat damit einen fertigen, benannten MCP-Gegenpart. Aufruf-Namen für die Implementierung: `novamira-adrianv2/wpcode-list`, `-get`, `-create`, `-update`, `-set-status`, `-duplicate`, `-delete`, `-check-setup`.
2. **`WordPress_mcp_adrian` = `novamira-adrianv2`, bestätigt identisch.** Beide Repos geklont und verglichen: identische Verzeichnisstruktur, identischer Plugin-Header (Version 1.10.1), identische letzte 5 Commit-Hashes. `WordPress_mcp_adrian` ist schlicht der alte Name des Repos vor einem GitHub-Rename — keine zwei Plugins. Doku-Links auf `WordPress_mcp_adrian/ARCHITECTURE.md` können 1:1 auf `novamira-adrianv2/ARCHITECTURE.md` normalisiert werden.
3. **Meta-/Ops-Tooling — Entscheidung: eigener `tools/`-Ordner, nicht in den 7 Packages.** Kein npm-Workspace-Package, kein Publish, nur lokale Scripts (`quarterly-audit.ts`, `measure-quality-metrics.ts`, `profile-pipeline.ts`, `lint-test-count.ts`, `inspect-v4-schemas.ts`, `sync-schema.ts`, `audit-resilience.ts`, `foundation-resilience.ts`, `auto-memory-save.ts`) — analog zu `tools/framer-export` im Quell-Repo. Wer sie braucht, ruft sie per `tsx tools/<script>.ts` auf; sie landen nie im Bundle von `elconv convert`.
4. **`session-init.ts` ist NICHT redundant zu `cmd-session.ts` — unterschiedliche Aufgabe, jetzt korrekt zugeordnet.** `cmd-session.ts` (54 Zeilen) verwaltet nur simplen Session-State (Target/URL/PostId/Phasen). `session-init.ts` (278 Zeilen) ist tatsächlich eine **Umgebungs-Readiness-Probe**: prüft MCP-Erreichbarkeit, WP-Helper-Klassen, Elementor-Version/Pro-Status, Atomic-Verfügbarkeit und die V4-Experiment-Flags (`e_atomic_elements`, `e_opt_in_v4`, `e_variables`, `e_classes`) und liefert einen strukturierten `McpCallEntry[]`-Report. Das gehört inhaltlich zu **Phase 55** (Framer-Preflight-Gates) — Quellenliste dort um `session-init.ts` ergänzt, `cmd-session.ts` bleibt unverändert.
5. **`tools/framer-export` — Entscheidung: eigenes 8. Workspace-Package `packages/framer-export`.** Macht `npm ci` allein ausreichend und behebt strukturell den in Teil 3 genannten fehlgeschlagenen Test (fehlender separater Install-Schritt).
