...
- [x] **State-/Contract-Konsistenzcheck (Doctor-Erweiterung):** `checkWizardStateConsistency()` in `cmd-doctor.ts` kreuz-validiert die State-Datei gegen den validierten Contract daneben: Target-Kohärenz (hand-editiertes Paar kann nicht stillschweigend abweichen), Phasen-/Exit-Code-Kohärenz (Exit 0 ⇒ State bei `done`; fehlgeschlagener Run ⇒ State sitzt auf der failed-Phase), `completedPhases` vs. Per-Phase-Status (`ok`-Phase muss completed sein — `done` ausgenommen, da nie in `completedPhases`; completed-Phase darf nicht `pending`/`failed` sein) und Timestamp-Kohärenz (State `updatedAt` > Contract-Mtime + 5s-Toleranz ⇒ Contract veraltet, z. B. fehlgeschlagener Contract-Write). Eine fehlende State-Datei ist legitim überspringbar (Contract bleibt auditierbar), eine kaputte State-Datei ist ein Finding (Exit 1). Der `WizardContractCheckReport` trägt `stateCheck` (stateFileReadable/stateValidJson/state/issues/consistent/stateNewerThanContractSeconds) und `ok`/Exit-Code schlagen bei lesbarer, aber inkonsistenter State-Datei auf 1 um. 6 neue Tests: konsistentes fertiges Paar (Exit 0), laufend vs. Exit 0 (Exit 1), veralteter Contract (Exit 1), ok-nicht-completed (Exit 1), Target-Mismatch (Exit 1), JSON-Report mit `stateCheck`; die bestehenden 6 Contract-Tests bleiben grün (fehlende State-Datei ⇒ Check übersprungen).
