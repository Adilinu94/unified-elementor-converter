...
**Status (umgesetzt):** 8 diff-Dateien nach `qa/src/diff/` + `ssim.ts` nach `qa/src/ssim.ts` portiert. Imports umgebogen: `@elconv/core` (BBox/IgnoreRegion/BlockDiffResult/ComprehensiveDiffResult/RunComprehensiveDiffFn/ViewportScreenshot/MultiViewportReport + `oklchHexToRgb as hexToRgb`/`rgbToOklch`/`Oklch`), `@elconv/extractors` (SectionInfo). Npm-Deps in qa/package.json: sharp ^0.33.5, pixelmatch ^6.0.0, pngjs ^7.0.0, ssim.js ^3.5.0, @elconv/extractors, @types/pngjs, @types/pixelmatch. qa/tsconfig: `lib: ["ES2022","dom"]` (für Playwright page.evaluate DOM-Types) + reference `../extractors`. `visual-diff.ts` (semantischer Reporting-Layer) bleibt unverändert (komplementär, kein Doppelcode). 8 Tests portiert (30 Tests). `tsc --build`+`vitest run` grün (545 passed | 2 skipped).
...
**Status (umgesetzt):** 7 von 9 Dateien portiert: `strictness.ts`, `pixel-diff.ts` (umbenannt von source visual-diff.ts; Monorepo-visual-diff.ts bleibt als komplementärer Semantic-Layer), `issue-detector.ts`, `html-report.ts`, `vision-qa.ts` (delegiert an `@elconv/core` AI-Tasks), `pixel-element-resolver.ts`, `real-fixers.ts`. Fehlende `AutoFixReport`/`AutoFixFixer`/`AutoFixFixerContext`/`RoundResult`-Interfaces in `auto-fix.ts` als Typ-Stubs ergänzt (Laufzeit-Implementierung folgt später). **Deferred:** `cross-validator.ts` (braucht V3Element/V4AtomicElement aus Builder → Phase 43+), `acceptance.ts` (braucht `visual-capture.ts` → noch nicht portiert). ssim.ts bereits in Phase 39 erledigt. 1 Test portiert (strictness, 16 Tests). `tsc --build`+`vitest run` grün (561 passed | 2 skipped).
...
**Status (umgesetzt):** Kein neues `core/validator/` — das Monorepo hatte den Validator bereits besser aufgeteilt (Runner+`formatGuardReport` in `core/guards.ts`, V3-Guards in `target-v3/guards.ts`, V4-Guards in `target-v4/guards.ts`). Portiert wurden die **fehlenden** Guards aus `validator/json-guard.ts`: V3 `G4:breakpoint-coverage`, `G5:image-url-present`, `G7c:flex-row-child-width` (nutzt `findFlexRowStackRisks` aus normalize.js); V4 `G11:style-classes-binding` (an V4TreeNode-Shape adaptiert: classes aus `settings.classes` $$type-Envelope statt `el.classes`). Neue Wrapper `runV3Guards(tree, threshold=85)` + `runV4Guards(tree, threshold=85)`. 2 Tests portiert/adaptiert (26 Tests). `tsc --build`+`vitest run` grün (587 passed | 2 skipped).
...
**Status (umgesetzt):** 10 von 11 Dateien portiert (statt 9: `adaptive-scroll.ts` + `spec-schema.ts` als Pflicht-Deps mitgezogen). Browser-Layer-Dateien nach `extractors/src/browser/` (adaptive-scroll, pseudo-state-capture, animation-property-extractor, custom-property-extractor, keyframes-discovery — verbatim, `./types.js` löst auf), Spec-Dateien flach nach `extractors/src/` (spec-schema, spec-builder, spec-md-writer, responsive-matrix, extract-pipeline). `robots-check.ts` aus Phase 43 vorgezogen → `extractors/src/assets/` (extract-pipeline braucht es). `mergeSmallSections`/`areMergeable`/`MergeThreshold`/`DEFAULT_MERGE_THRESHOLD` in vorhandenen `browser/section-detector.ts` übernommen. `BrowserExtractionResult.designTokens?: DesignTokens` (optional) ergänzt. **Deferred:** `browserbase-extractor.ts` (braucht `@browserbasehq/sdk`-npm-Dep + nicht exportiertes `extractFromPage` — nachziehen wenn Cloud-Browser-Support gebraucht wird). 6 Tests portiert (90 Tests inkl. robots-check). `tsc --build`+`vitest run` grün (677 passed | 2 skipped).
...
**Status (umgesetzt):** Alle 3 Dateien portiert — `robots-check.ts` bereits in Phase 42 vorgezogen (von `extract-pipeline` konsumiert); `svg-downloader.ts` + `favicon-og-downloader.ts` nach `extractors/src/assets/` portiert. Anpassung: `undici`-Request durch globales `fetch` + AbortController-Timeout ersetzt (Monorepo-Idiom, keine neue Dependency; `p-limit` war vorhanden). Beide Quell-Tests portiert (svg-downloader 11 Tests, favicon-og-downloader 8 Tests — echte lokale HTTP-Testserver, kein Mock nötig). `tsc --build`+`vitest run` grün (696 passed | 2 skipped).
...
|------:|:---:|-------|--------|-------|
| 35 | F | Contracts | ✅ done (811b1e0) | core/contracts, 5 Dateien |
| 36 | A | Analyzer + Tokens | ✅ done (3a79110) | core/analyzer + analysis/token-mapping, 96 Tests |
| 37 | G | Design-System | ✅ done (c758bb0) | core/design-system, 19 Tests |
| 38 | B | AI-Tasks | ✅ done (544ae7c) | core/ai/tasks, 5 Tasks, 14 Tests |
| 39 | D | QA Diff-Engine | ✅ done (aaabf56) | qa/diff + ssim.ts, 30 Tests, echte pixelmatch/sharp/SSIM |
| 40 | E | QA-Extras | ✅ done (7b1c3f5) | 7/9 Dateien; cross-validator + acceptance deferred |
| 41 | H | Validator | ✅ done (bbcf194) | G4/G5/G7c/G11 + runV3Guards/runV4Guards in target-v3/v4 |
| 42 | I | Extractor-Extras | ✅ done | 10/11 Dateien; browserbase deferred; robots-check vorgezogen |
| 43 | J | Scraper-Extras | ✅ done | svg+favicon-og portiert (undici→fetch); robots-check schon in P42 |
...
### Offene Punkte / Technische Schulden (Stand nach Phase 41)

1. **Healing-Loop noch nicht „echt“:** `qa/src/auto-fix.ts` enthält für `AutoFixReport`/`AutoFixFixer`/`AutoFixFixerContext`/`RoundResult` nur **Typ-Stubs** — die `runAutoFix()`-Engine (Quelle: `site-clone-to-v3/src/qa/auto-fix.ts`, 359 Z., inkl. `summarizeReport`/`buildDefaultFixers`) ist noch nicht portiert. `real-fixers.ts` läuft bis zur MCP-Integration (Phase 47) nur im DRY-RUN.
2. **Deferred aus Phase 40:** `cross-validator.ts` (braucht V3Element/V4AtomicElement-Builder-Typen → nach Phase 43/44 nachziehen) und `acceptance.ts` + `visual-capture.ts` (nicht portiert).
3. **AI-Provider fehlen:** `createAIRouter()` + `ClaudeProvider`/`Gpt4VisionProvider` aus `site-clone-to-v3/src/ai-engine/` (provider.ts/index.ts) sind noch nicht in core — ohne sie ist der AIRouter nicht produktiv instanziierbar (Tasks selbst sind da).
4. **Doppelte Typdefinitionen:** `IssueType`/`IssueSeverity` existieren in core (`ai/tasks/vision-qa.task.ts`) UND qa (`strictness.ts`/`issue-detector.ts`). Strukturell identisch; Konsolidierung (qa importiert aus core) steht aus — Vorsicht bei kombinierten Barrel-Re-Exports (ambiguous export wird silent gedroppt).
5. **Drei Diff-Layer:** `qa/visual-diff.ts` (semantischer Mock-Layer), `qa/pixel-diff.ts` (einfacher pixelmatch-Wrapper), `qa/diff/` (comprehensive Engine). Funktional komplementär; optionales Konsolidierungs-Cleanup (healing-loop nutzt noch `createMockDiffResult`).
6. **sharp auf frischen Maschinen:** npm-allowScripts blockierte das sharp-Install-Skript; Binary kam aus dem System-Cache. Auf frischer CI ggf. `npm install-scripts approve sharp` nötig.
7. **Quell-Tests noch nicht portiert (Phase 40):** issue-detector, vision-qa, html-report, real-fixers, pixel-element-resolver Tests aus `site-clone-to-v3/tests/unit/qa/` (nur strictness.test.ts portiert).

**Version:** 1.0.0 (Basis) → 2.0.0 (nach vollständiger Portierung, Phase 50 grün)
**Tests:** 50 Dateien, 587 Tests, 2 skipped (Stand nach Phase 41, Commit bbcf194)
