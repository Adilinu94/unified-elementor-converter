...
import { VIEWPORTS, calculateDiffScore, classifySeverity, mapToSemanticRegion, generateDiffMessage, aggregateRegions, createMockDiffResult, compareDiffResults, resetFixIds, FIX_CONFIGS, calculateFixPriority, createFixAction, createPriorityQueue, getNextBatch, markFixApplied, markFixVerified, isQueueComplete, getRemainingCount, inferFixType, resetProbeIds, probeSharedIds, probeElementCount, probeNestingDepth, probeWidgetTypes, runStructuralProbes, createViewportRegion, detectResponsiveIssues, aggregateViewportMatrix, createMockViewportMatrix, getViewportRecommendations, RESPONSIVE_THRESHOLDS, } from '@elconv/qa';
...
describe('Viewport Matrix', () => {
    ...
    it('generates recommendations for issues', () => {
        const matrix = createMockViewportMatrix({ desktop: 1, tablet: 2, mobile: 20 }, [makeRegion({ diffPercent: 20, severity: 'critical' })]);
