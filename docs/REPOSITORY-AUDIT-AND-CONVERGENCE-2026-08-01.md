...
| O-03 | P1 | Unified Deploy | TEILWEISE / Unified | `tree-chunk` (Plugin fc26eb6, `tree-chunk-start/append/commit`, ~2 KB/Chunk, 5 MB Cap) ist als kanonische Large-Strategie für MCP-only Clients an den Orchestrator angeschlossen und bleibt bis zur Schema-Verifikation bewusst `unavailable` (V3), V4 `tree-chunk` bewusst `unavailable` bis V4 Elementor_data Export geklärt; `upload-php`/`split` bleiben als http-fähige Alternative gegated; `direct` bleibt der einzige live pushbare Pfad. Offline: `tree-chunk-deploy.ts` + `large-deploy-plan.ts` frieren Verträge mit Registry-Guard ein; Fixtures + Mock-Tests sichern Bandenwahl/Vertrag/Gate. | Erst nach verifizierten Schemas dürfen große Trees über `tree-chunk` snapshot-/rollback-fähig deployt werden; kein Erfolg ohne verifizierten Read-back + Cache-Clear. |
