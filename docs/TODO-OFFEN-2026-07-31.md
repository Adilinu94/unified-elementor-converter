...
- [x] **Verifikationspfad `elconv doctor --verify-large-deploy`:** `packages/mcp/src/large-deploy-verification.ts` leitet die Erwartungen **direkt aus `planLargeDeploy` ab** (keine duplizierte Tabelle, die still vom eingefrorenen Vertrag abweichen könnte; alle geplanten Abilities laufen durch `assertPlanUsesKnownAbilities`), ruft die **live Input-Schemas der vier Vertrags-Abilities** per `mcp-adapter-get-ability-info` ab (Transport-Meta-Tool wie `discover-abilities`, bewusst nicht registry-gated; `McpAdapter.getAbilityInfo()` als Spiegel von `listAbilities`) und gleicht sie ab: erwartete Parameter vorhanden, und Deploy-Abilities müssen `mode` mit Enum `replace`/`append` deklarieren. Das Live-Payload-Shape wird **defensiv** erkannt (bare `properties` oder `input_schema`/`inputSchema`/`parameters`/`schema`-Wrapper); unerkannte Shapes werden ehrlich als nicht-verifiziert gemeldet statt geraten. Der Report trägt `requiresLiveRoundtrip: true` als **Literal** — selbst ein vollständig schema-verifizierter Check kann das produktive Gate nicht öffnen (`deploy.ts` weist weiterhin `capability-unavailable`, `requiresSchemaVerification` bleibt Literal-`true`). `cmd-doctor.ts` nutzt den gemeinsamen `resolveDoctorAuth`-Helfer (gleicher Auth-Vertrag wie `--sync-abilities`), `--json` emittiert den maschinenlesbaren `LargeDeployVerificationReport` (ok/checks mit status+missingParams+mode/issues/exitCode/authMode/timestamp); Exit-Codes 0 verifiziert · 1 nicht verifiziert/unavailable · 2 Usage. 16 neue MCP-Tests (`large-deploy-verification.test.ts`: Ableitung aus dem Vertrag, Shape-Erkennung, Mode-Enum-Checks, unrecognized shape, unavailable, Gate-Literal) + 4 neue CLI-Tests (`cmd-doctor.test.ts`: Usage, verifizierter JSON-Report, unavailable, Human-Regression) + 1 Adapter-Test (`getAbilityInfo` übergibt `ability_name`, nicht registry-gated, expliziter Timeout).

**Status:** `executeDeploy` gated `upload-php`/`split` weiterhin ohne explizites `largeDeployVerified` mit `capability-unavailable` und führt dann keinen MCP-Write aus; die serverseitigen Upload-/Append-Schemas bleiben bis zur Verifikation gegen das echten Testziel unverifiziert. Der neue `--verify-large-deploy`-Check ist die diagnostische Vorstufe: Er liefert die Evidenz, ob die Live-Schemas den eingefrorenen Vertrag erfüllen — die produktive Freischaltung erfordert danach zusätzlich den kontrollierten Live-Roundtrip (Snapshot → kleiner Deploy → Read-back → Cache-Clear → Render-QA → Rollback) und den bewussten Code-Entscheid (Literal kippen + Gates entfernen).
- [x] **`runPlannedDeploy` in `executeDeploy` verdrahtet (Unlock-Vorbereitung, Gate bleibt zu):** `DeployOptions.largeDeployVerified` — ein explizites Opt-in, das produktiv niemand setzt (die CLI übergibt es nicht). Ohne das Opt-in verhält sich `executeDeploy` für `upload-php`/`split` exakt wie bisher (`capability-unavailable`, 0 MCP-Calls, Fehlerstrings unverändert). Nur mit `largeDeployVerified: true` läuft der eingefrorene geplante Pfad: `planLargeDeploy` → `runPlannedDeploy` mit der Transaction-ID (Read-back + Cache-Clear pro Chunk sind Teil des Vertrags — `skipVerify` greift hier bewusst nicht), bei Erfolg Commit + `chunks` im Report, bei Fehlschlag `deploy-failed` (Transaction wird gefailt, kein Commit). Der Unlock-Commmit aus Phase 4 reduziert sich damit auf: Plan-Literal kippen, Opt-in in der CLI setzen, Gate-Tests umstellen. 4 neue Tests in `large-deploy-offline.test.ts`: voller upload-php-Pfad durch `executeDeploy` (v3, replace→append, transaction_id), voller split-Pfad (v4, 20er-Chunks), deploy-failed ohne Commit, Gate ohne Opt-in weiter geschlossen. **+2 Regressionstests:** (a) Mid-Plan-Chunk-Fehler auf Orchestrierungsebene — Chunk 0 wird deployt UND verifiziert, Chunk 1 scheitert an beiden Retries → `deploy-failed`, Transaction-Status `failed` (kein Commit), der verifizierte Checkpoint wird persistiert, und ein zweiter `executeDeploy`-Aufruf mit `resumeTransactionId` überspringt Chunk 0 und komplettiert Chunk 1 als `append` (Transaction `committed`); (b) **V3-Normalisierung vor dem Planen:** V3 wird vor Strategieauswahl und `planLargeDeploy` normalisiert, und ein Flex-Row-Fixture pinnt die injizierte Breite im geplanten Deploy-Payload.
- [ ] **Verifikations-Checkliste:** `docs/LARGE-DEPLOY-VERIFICATION-2026-08-04.md` — vollständiger, abhakbarer Fahrplan von der Testziel-Freigabe (isoliertes WP + WPCode-Entscheidung + Mutationsfreigabe + Snapshot-Recht) über die Read-only-/Schema-Verifikation (`--sync-abilities`, `--verify-large-deploy` mit JSON-Evidenz) und den kontrollierten Live-Roundtrip (Baseline-direct → Multi-Viewport-QA → Rollback-Test → erster echter `upload-php` → `split`, je mit Read-back/Cache-Clear-Beobachtungschecklisten) bis zum bewussten Code-Unlock (Plan-Literal kippen, `deploy.ts`-Gates durch `runPlannedDeploy` ersetzen, CLI-Texte/Help aktualisieren, Gate-Tests umstellen, neuer freigeschalteter Regressionstest), Post-Unlock-Nachweis und Definition of Done.
...
- [x] **`readWizardContract` produktiv verdrahtet (O-12):** `reportWizardContractOnResume(stateFile)` in `cmd-wizard.ts` liest beim `--resume` den `wizard-contract.json` neben der State-Datei, soft-migriert Pre-O-12-Artefakte und bestätigt das Ergebnis sichtbar (`✓ valid (schemaVersion 1)` / `✓ migrated from pre-O-12 (N change(s) applied)` / `⚠ invalid` mit Fehlerzeilen). Best-effort: fehlende Datei bleibt still, ein ungültiger Contract blockiert den Resume nie (Exit-Code unverändert, die State-Datei ist die Resume-Quelle). 4 neue CLI-Tests in `cmd-wizard.test.ts` decken valide, migriert, invalid (Warnung ohne Block) und still-bei-fehlend ab.
- [x] **Mock-Remote-Adapter an den `WizardRemoteStatePort` gebrückt (O-04/O-12):** `bridgeRemoteStateAdapter()` in `remote-state.ts` mappt den ergebnis-shape-adapter auf den werfenden/`null`-Port-Vertrag — `notFound` → `null` („start fresh“, nie ein Fehler), jede andere Nicht-Erfolg-Antwort (inkl. ungültigem Envelope) → geworfener Fehler (nie stilles Coerzen). `WizardDependencies.remoteState` akzeptiert jetzt `WizardRemoteStateAdapter | WizardRemoteStatePort`; `cmdWizard` normalisiert genau einmal über `toRemoteStatePort()` und fädelt den Port durch Resume-Load, `persistWizardState` und `runWizardStateMachine`; `persistContract` berichtet ehrlich, wenn ein injizierter Adapter `verified: false` meldet (`remote-state adapter injected but not verified: <reason>`). Damit ist Resume über `--remote-state-key` im Wizard offline end-to-end testbar (save/load/resume + Envelope-Validierung) — ohne produktive MCP-Freischaltung. 3 neue Wizard-Tests (`cmd-wizard.test.ts`: Done-Run rein aus dem Remote-Key, Zwei-Lauf-Save→Resume-Roundtrip durch dieselbe Mock-Instanz inkl. Envelope-`$schema`, invalid-envelope → Exit 2) und 2 neue Bridge-Tests (`remote-state.test.ts`: Roundtrip, notFound→null, unavailable→throw).
- [x] **`elconv doctor --wizard-contract <state-file>` (O-12-Ergänzung):** `checkWizardContract()` in `cmd-doctor.ts` liest den `wizard-contract.json` neben einer Wizard-State-Datei über denselben `readWizardContract`-Pfad wie die Resume-Verdrahtung (soft-migriert Pre-O-12-Artefakte, validiert gegen das versionierte Schema) — ohne den Wizard zu resumen. Standalone (kein `--target` nötig), Exit-Codes: 0 valide/migriert, 1 invalid/fehlend/inconsistent, 2 Usage (`--wizard-contract` ohne Pfad). `--json` emittiert den maschinenlesbaren `WizardContractCheckReport` (ok/contractPath/migrated/notes/errors/exitCode/contractSummary mit Phasen-Status, Remote-State-Gate). 6 neue Tests in `tests/unit/cli/cmd-doctor.test.ts` decken valide, migriert, invalid (Exit 1), fehlend (Exit 1), Usage (Exit 2) und JSON-Report ab.
- [x] **State-/Contract-Konsistenzcheck (Doctor-Erweiterung):** `checkWizardStateConsistency()` in `cmd-doctor.ts` kreuz-validiert die State-Datei gegen den validierten Contract daneben: Target-Kohärenz (hand-editiertes Paar kann nicht stillschweigend abweichen), Phasen-/Exit-Code-Kohärenz (Exit 0 ⇒ State bei `done`; fehlgeschlagener Run ⇒ State sitzt auf der failed-Phase), `completedPhases` vs. Per-Phase-Status (`ok`-Phase muss completed sein — `done` ausgenommen, da nie in `completedPhases`; completed-Phase darf nicht `pending`/`failed` sein) und Timestamp-Kohärenz (State `updatedAt` > Contract-Mtime + 5s-Toleranz ⇒ Contract veraltet, z. B. fehlgeschlagener Contract-Write). Eine fehlende State-Datei ist legitim überspringbar (Contract bleibt auditierbar) und wird im Report weggelassen (kein erfundenes `consistent: false`); eine kaputte State-Datei ist ein Finding (Exit 1). Der `WizardContractCheckReport` trägt `stateCheck` (stateFileReadable/stateValidJson/state/issues/consistent/stateContractDeltaSeconds) und `ok`/Exit-Code schlagen bei lesbarer, aber inkonsistenter State-Datei auf 1 um. 6 neue Tests: konsistentes fertiges Paar (Exit 0), laufend vs. Exit 0 (Exit 1), veralteter Contract (Exit 1), ok-nicht-completed (Exit 1), Target-Mismatch (Exit 1), JSON-Report mit `stateCheck`; die bestehenden 6 Contract-Tests bleiben grün (fehlende State-Datei ⇒ Check übersprungen).
- [x] **Auto-Modus `elconv doctor --wizard-contracts <dir>`:** `findWizardContractPairs()` in `cmd-doctor.ts` findet rekursiv alle State-/Contract-Paare über den Contract als Anker (jeder Contract wird als `<state-file>.contract.json` geschrieben → Suffix-Stripping ergibt die State-Datei; expliziter rekursiver Walk statt `readdirSync(recursive)` für runtime-unabhängige Pfadbildung, dedupliziert + sortiert). `checkWizardContractsInDir()` prüft jedes Paar über denselben `checkWizardContract`-Pfad (Contract + Konsistenz) und aggregiert `WizardContractsDirReport` (dir/errors/checks/summary total-ok-failed/exitCode). Exit-Codes: 0 alle Paare ok (und ≥ 1 gefunden), 1 irgendein Paar fehlgeschlagen ODER nichts gefunden ODER Verzeichnis fehlt (Scan-Level-Fehler im Report), 2 Usage (`--wizard-contracts` ohne Pfad). `--json` emittiert den aggregierten Report. 6 neue Tests: gemischtes Verzeichnis (total 2, ok 1, failed 1), alles-ok (Exit 0), Rekursion in verschachtelten Unterverzeichnissen, leeres Verzeichnis (Exit 1, „no wizard contract pairs found“), fehlendes Verzeichnis (Exit 1, `Directory not found` im JSON), Usage (Exit 2).
- [x] **`--sync-abilities --json` (maschinenlesbarer Ability-Drift):** `buildAbilitySyncReport()` in `cmd-doctor.ts` leitet den `AbilitySyncReport` deterministisch aus `diffAbilityRegistry` ab — `ok: true` mit inSync/liveCount/snapshotCount/addedOnServer/removedFromServer/nowAvailable, authMode (`target-name` | `mcp-url-auth-env`), optionalem targetName, ISO-Timestamp und Exit-Code (0 in-sync, 1 Drift); bei Discovery-Fehler `ok: false` mit `error` und Exit 1. `syncAbilities()` akzeptiert einen injizierbaren `listAbilities`-Port (Offline-Tests, kein Netz) und emittiert mit `--json` nur den Report. Usage-Fehler (Exit 2) bleiben stderr-only ohne Report. 7 neue Tests: Usage (Exit 2), injizierter In-Sync-JSON-Report (Exit 0, alle Felder), Drift added/removed/nowAvailable (je Exit 1), Discovery-Fehler-JSON (Exit 1), reiner Builder mit targetName-Metadaten.
